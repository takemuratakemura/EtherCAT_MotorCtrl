...
/* ステータス変更関連 */
uint16_t statusword = 0;   // ステータス今回地
uint16_t statusword_l = 0; // ステータス前回値
...
/* ステータス変更 */
            ...
            else if(statusword == 520 || statusword == 536){
                datasize = sizeof(error_code);
                ec_SDOread(1, 0x603F, 0x00, FALSE, &datasize, &error_code, EC_TIMEOUTRXM);
                printf("★★★ Status 536 / Error code: 0x%04X\n", error_code);
                controlword = 0x80;
                ec_SDOwrite(1, 0x6040, 0x00, FALSE, sizeof(controlword), &controlword, EC_TIMEOUTRXM);
                osal_usleep(100000);
            }
...
/* ステータスが変化したときだけ表示 */
            if(statusword_l != statusword){
                ...
                printf("(%d)\n", statusword);
                printf("sent controlword: 0x%02X\n", controlword);
            }
